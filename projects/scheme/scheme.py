...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        oprn = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(oprt, oprn, env)
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        name_value = bindings.first
        validate_form(name_value, 2, 2)
        names = Pair(name_value.first, names)
        values = Pair(scheme_eval(name_value.rest.first, env), values)
