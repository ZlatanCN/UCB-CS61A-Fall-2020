...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            place_is_container = isinstance(place.ant, ContainerAnt)
            self_is_container = isinstance(self, ContainerAnt)
            if place_is_container and ContainerAnt.can_contain(place.ant, self):
                ContainerAnt.contain_ant(place.ant, self)
            elif self_is_container and ContainerAnt.can_contain(self, place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not beehive:
            is_in_range = (self.min_range <= how_far <= self.max_range)
            exist_bees = (len(current_place.bees) != 0)
            if is_in_range and exist_bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees_lst_copy = list(self.place.bees)
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            amount += self.damage
        for bee in bees_lst_copy:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if len(bees_in_place) == 0:
            return
        elif self.digesting == 0:
            a_bee = rANTdom_else_none(bees_in_place)
            self.eat_bee(a_bee)
        else:
            self.digesting -= 1
        # END Problem 6
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt just do nothing but be stung."""
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        self.num = QueenAnt.serial_num
        QueenAnt.serial_num += 1
        # END Problem EC
        ...
        if self.num == 0:
            ...
            while current_place is not None:
                if current_place.ant is not None:
                    if current_place.ant.double_damage == False:
                        current_place.ant.damage *= 2
                        current_place.ant.double_damage = True
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is not None:
            return True if self.place.ant.blocks_path is True else False
        else:
            return False
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.go_ahead == False and self.place.entrance != gamestate.beehive:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        current_place = self.place
        while current_place is not beehive:
            bees_lst_copy = list(current_place.bees)
            if current_place.ant != None and current_place.ant != self:
                all_insects[current_place.ant] = distance
                for bee in bees_lst_copy:
